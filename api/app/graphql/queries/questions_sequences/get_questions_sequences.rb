module Queries
  module QuestionsSequences
    class GetQuestionsSequences < Queries::BaseQuery
      type Types::QuestionsSequenceType.connection_type, null: false
      ...
      def resolve(project_id:, search_term: '')
        ...
        if search_term.present?
          project.questions_sequences.includes(:instances).search(search_term, project.language.code)
        else
          project.questions_sequences.includes(:instances).order(updated_at: :desc)
        end
