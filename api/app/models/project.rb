...
class Project < ApplicationRecord
  ...
  def create_default_variables
    ...
    bmi = variables.create!(label_translations: Hash[Language.all.map(&:code).collect { |k| [k, I18n.t('variables.default_variables.bmi', locale: k)] } ], type: 'Variables::BasicMeasurement', stage: Variable.stages[:registration], step: Variable.steps[:basic_measurements_step], answer_type_id: 5, formula: "[#{weight.id}] / (([#{height.id}] / 100) * ([#{height.id}] / 100))", is_default: true)
