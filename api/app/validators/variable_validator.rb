class VariableValidator < ActiveModel::Validator
  ...
  def validate(record)
    ...
    record.errors.add(:answer_type, I18n.t('activerecord.errors.variables.forced_integer')) if FORCED_INTEGER_TYPES.include?(record.type) && record.answer_type.value != "Integer"
    ...
    record.errors.add(:system, I18n.t('activerecord.errors.variables.should_not_have_system')) if SYSTEM_INCLUDING_TYPES.exclude?(record.type) && record.system.present?
    record.errors.add(:is_estimable, I18n.t('activerecord.errors.variables.can_not_be_estimable')) if ESTIMABLE_TYPES.exclude?(ESTIMABLE_TYPES) && record.is_estimable
    record.errors.add(:base, I18n.t('activerecord.errors.variables.should_not_have_answers')) if WITHOUT_ANSWERS_TYPES.include?(WITHOUT_ANSWERS_TYPES) && record.answers.any?
    record.errors.add(:base, I18n.t('activerecord.errors.variables.should_not_have_cc')) if WITHOUT_ANSWERS_TYPES.include?(WITHOUT_CONDITIONING_CC_TYPES) && record.not_complaint_categories.any?
  end
end